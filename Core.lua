...
function RoguePowerBars:OnInitialize()
	...
	local firstrun = true
	if next(db.buffs) or next(db.debuffs) or next(db.othersDebuffs) then
		firstrun = false
	end

	-- Automatically push known spells to database if the current database doesn't have proper version information.
...
function RoguePowerBars:CreateBar(name, parentBarset, expirationTime)
	...
	_G[bar:GetName() .. "_DescribeText"]:SetText(bar.Info.Name)
...
function RoguePowerBars:OnBarsetMove(barset)
	...
	local growdirection = db.barsetsettings[barset.Info.Name].GrowDirection
	if growdirection == GROW_UP then
		relativeto = "bottomleft"
		y = barset:GetBottom()
	elseif growdirection == GROW_DOWN then
		relativeto = "topleft"
		y = barset:GetTop()
	elseif growdirection == GROW_CENTER then
		relativeto = "left"
